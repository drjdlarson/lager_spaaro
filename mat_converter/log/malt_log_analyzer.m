...
xy_vel_sq = xy_vel.^2;
gnd_speed = xy_vel_sq(:,1) + xy_vel_sq(:,2);
gns_speed = gnd_speed.^0.5;
...
plot (mav_pos_ned(:,1), mav_pos_ned(:,2),'r')
xlim([0 8])
ylim([0 5])

figure(4)
plot (sys_time_s, mav_pos_ned(:,2) - 1)
hold on
plot (sys_time_s, vms_aux14 .* sign(vms_aux15))
grid on
grid minor
